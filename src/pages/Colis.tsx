...
function ErreursRemise() {
  ...
  async function remettreEnCours(id: string) {
    await supabase
      .from('colis_erreurs_remise')
      .update({ resolu: false, resolu_par: null, resolu_le: null })
      .eq('id', id)
    charger()
  }
  ...
  async function enregistrerReponse(id: string) {
    ...
    await supabase.from('colis_erreurs_remise').update({ reponse: reponse.trim() || null }).eq('id', id)
  ...
  async function supprimer(id: string) {
    await supabase.from('colis_erreurs_remise').delete().eq('id', id)
    charger()
  }

  function ouvrir(e: ColisErreurRemise) {
    if (ouverte === e.id) {
      setOuverte(null)
    } else {
      setOuverte(e.id)
      setReponse(e.reponse ?? '')
    }
  }
        ...
        <div className="space-y-2">
          ...
          {enCours.map((e) => (
            <CarteErreur
              ...
              onEnregistrerReponse={() => enregistrerReponse(e.id)}
              onMarquerResolu={() => marquerResolu(e.id)}
              onSupprimer={() => supprimer(e.id)}
            />
          ))}
        </div>
      )}
        ...
        <div className="space-y-2">
          ...
          {resolues.map((e) => (
            <CarteErreur
              ...
              onChangeReponse={setReponse}
              onEnregistrerReponse={() => enregistrerReponse(e.id)}
              onRemettreEnCours={() => remettreEnCours(e.id)}
              onSupprimer={() => supprimer(e.id)}
            />
          ))}
        </div>
  ...
  return (
    <div
      className={`rounded-xl p-3 ${
        resolue ? 'bg-white shadow-sm opacity-80' : 'bg-corail/10 border border-corail/30'
      }`}
    >
      <button onClick={onOuvrir} className="w-full text-left">
        <p className={`text-sm ${resolue ? 'line-through' : ''}`}>{e.description}</p>
        <p className="text-xs text-encre/40 mt-1">
          Signalé par {e.profiles?.full_name ?? '—'} · {formatDate(e.signale_le)}
        </p>
        {e.reponse && !ouverte && <p className="text-xs text-encre/60 mt-1 italic">Réponse : {e.reponse}</p>}
      </button>
      ...
      {ouverte && (
        <div className="mt-3 pt-3 border-t border-gray-100 space-y-2">
          <textarea
            value={reponse}
            onChange={(ev) => onChangeReponse(ev.target.value)}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!resolue && onMarquerResolu && (
              <button onClick={onMarquerResolu} className="text-sm font-medium text-havane underline px-2 py-2">
                Marquer comme résolu
              </button>
            )}
            {resolue && onRemettreEnCours && (
